...
Passing repository tests establishes contract and harness behavior only. It does not complete X-01 through X-13, authorize a formal result, satisfy a promotion gate, establish legal eligibility, or authorize real-money execution. Synthetic fixtures are never empirical evidence. Numerical X-07 outputs remain `PRELIMINARY`; the X-09 fixture may establish `HARNESS_PASS` while the formal experiment remains `EXPERIMENT_BLOCKED`.
