...
The POC compares the point-in-time market prior, standardized logistic regression, and histogram GBDT on identical game-grouped chronological walk-forward folds. The fit boundary rejects overlapping train/test games and any training prediction that is not strictly earlier than the test fold. Target, game identity, time, and availability columns cannot be model features. The frozen pregame prior is both the untouched benchmark and an explicit input to logistic and GBDT; it cannot be supplied again through `feature_columns`. Every state feature carries an availability timestamp no later than prediction time, while the pregame prior must be frozen no later than `game_start_at`.
