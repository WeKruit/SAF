...
The formal generator loads the experiment registry and refuses X-05 unless the `label_generation` scope is authorized, every required lock is resolved, code and data hashes are preregistered for that scope, and X-01 is complete. At runtime it deterministically hashes the exact quote sequence, the labels/contracts/registry code bundle, and every parameter plus the ask/bid/midpoint/suspension semantics. The quote-manifest lock must equal the runtime data hash; all parameter/rule locks must equal the runtime configuration hash; and the code/data hashes must equal the preregistered input record. Generated result references must subsequently pass Team H's append-only registration and temporal validation. Midpoint use, data substitution, code substitution, or unregistered parameter selection invalidates the run.
